<?php
include_once 'model/sa_apparel_colors_mdl.php';

class sa_apparel_colors_ctl extends sa_apparel_colors_mdl
{
	public $TempSession = "";
	...
	function getAllApparelColors(){
		return parent::getAllApparelColors_f_mdl();
	}
	
	function deleteApparelColor(){
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "delete-apa-col"){
				$id = parent::getVal("acId");
				
				parent::deleteApparelColor_f_mdl($id);
			}
		}
	}
	
	function apparelPagination(){
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "apparel_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				if(isset($keyword) && !empty($keyword)){
					$cond_keyword = "AND (
							apparel_color_name LIKE '%".trim($keyword)."%'
						)";
				}
				...
				$sql="
						SELECT count(id) as count FROM store_apparel_colors_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id, apparel_color, apparel_color_name, status FROM store_apparel_colors_master WHERE 1
						$cond_keyword
				...
				if(isset($_POST['pagination_export']) && $_POST['pagination_export']=='Y'){
					...
					if(!empty($all_list))
					{
						$sr = $sr_start;
							...
							$html .= '<td><div class="col-preview" style="background-color: '.	$single["apparel_color"].'"></div>'.$single["apparel_color_name"].'</td>';
							...
							$html .= '<td><button type="button" class="btn btn-primary waves-effect waves-classic btn-addedit-apa-col" data-href="sa-addedit-apparel-color.php?stkn='.parent::getVal("stkn").'&acid='.$single["id"].'">Edit</button><button type="button" class="btn btn-danger waves-effect waves-classic btn-confirm-delete-apa-col" data-id="'.$single["id"].'">Delete</button></td>';
