...
class sa_designs_ctl extends sa_designs_mdl
{
	...
	function getAllDesigns(){
		return parent::getAllDesigns_f_mdl();
	}
	
	function deleteDesign(){
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "delete-design"){
	...
	function designsPagination(){
		global $s3Obj;
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "design_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				$sql="
						SELECT count(id) as count FROM store_free_design_product_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id, product_title, product_image, status FROM store_free_design_product_master WHERE 1
						$cond_keyword
				...
				if(isset($_POST['pagination_export']) && $_POST['pagination_export']=='Y'){
					...
					if(!empty($all_list))
					{
						...
						foreach($all_list as $single){
							...
							//$html .= '<td><img class="adj-img-sz" src="'.common::IMAGE_UPLOAD_URL.$single["product_image"].'"/></td>';
							$html .= '<td><img class="adj-img-sz" src="'.$s3Obj->getAwsUrl(common::IMAGE_UPLOAD_S3_PATH.$single['product_image']).'"/></td>';
							...
							$html .= '<td><button type="button" class="btn btn-primary waves-effect waves-classic btn-addedit-designs" data-href="sa-addedit-designs.php?stkn='.parent::getVal("stkn").'&did='.$single["id"].'">Edit</button><button type="button" class="btn btn-danger waves-effect waves-classic btn-confirm-delete-designs" data-id="'.$single["id"].'">Delete</button></td>';
