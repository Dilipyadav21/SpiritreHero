<?php
include_once 'model/sa_organizations_mdl.php';

class sa_organizations_ctl extends sa_organizations_mdl
{
	public $TempSession = "";
	...
	function getAllOrganizations(){
		return parent::getAllOrganizations_f_mdl();
	}
	
	function deleteOrganizations(){
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "delete-org"){
				$id = parent::getVal("orgId");

				parent::deleteOrganizations_f_mdl($id);
			}
		}
	}
	...
	function organizationPagination(){
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "organization_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				if(isset($keyword) && !empty($keyword)){
					$cond_keyword = "AND (
							organization_name LIKE '%".trim($keyword)."%'
						)";
				}
				...
				$sql="
						SELECT count(id) as count FROM store_organization_type_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id, organization_name, organization_short_code, status FROM store_organization_type_master WHERE 1
						$cond_keyword
				...
				if(isset($_POST['pagination_export']) && $_POST['pagination_export']=='Y'){
					...
					if(!empty($all_list)){
						...
						foreach($all_list as $single){
							...
							$html .= '<td>'.$single["organization_name"].'</td>';
							
							$html .= '<td>'.$single["organization_short_code"].'</td>';
							...
							$html .= '<td><button type="button" class="btn btn-primary waves-effect waves-classic btn-addedit-org" data-href="sa-addedit-organizations.php?stkn='.parent::getVal("stkn").'&oid='.$single["id"].'">Edit</button><button type="button" class="btn btn-danger waves-effect waves-classic btn-confirm-delete-org" data-id="'.$single["id"].'">Delete</button></td>';
