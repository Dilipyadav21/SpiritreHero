...
class sa_store_product_tags_ctl extends sa_addresses_mdl
{
	public $TempSession = "";
    ...
    function checkRequestProcess($requestFor){
        if($requestFor != ""){
            switch($requestFor){
				case "enabeld_or_disabled_store_tag":
					$this->enabeld_or_disabled_store_tag();
				break;
				case "add_store_product_tags":
					$this->add_store_product_tags();
                break;
				case "check_add_store_tag":
					$this->check_add_store_tag();
                break;
			}
        }
    }
	
	function StoreProductTagsPagination(){
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "store_product_tags_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				$sql="
						SELECT count(id) as count FROM store_product_tag_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id,tag,tag_status,created_on FROM store_product_tag_master WHERE 1
						$cond_keyword
						$cond_order
						LIMIT $start,$end
					";
	...
	function enabeld_or_disabled_store_tag(){
		$res = [];
		if(!empty(parent::getVal("method")) && parent::getVal("method") == "enabeld_or_disabled_store_tag"){
			$update_data = [
				'tag_status' => parent::getVal("tag_status")
			];
			$res = parent::updateTable_f_mdl('store_product_tag_master',$update_data,'id="'.parent::getVal('tag_id').'"');
		}
		echo common::sendJson($res,1);die();
	}

	function add_store_product_tags(){
		$res = [];
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "add_store_product_tags"){
			
				$tag_name = parent::getVal("tag_name");
				//$tag_status = parent::getVal("tag_status");
				$sql1 = 'SELECT tag FROM `store_product_tag_master` WHERE tag="'.$tag_name.'" ';
				...
				if(!empty($tag_data)){
					...
					$res=parent::insertTable_f_mdl('store_product_tag_master',$insertNewColorData);
				}
				echo common::sendJson($res,1);die();
			}
		} 
	...
	public function check_add_store_tag(){
		if (parent::isPOST()) {
			if (!empty(parent::getVal("method")) && parent::getVal("method") == "check_add_store_tag") {
				$tag= parent::getVal('tag');
				$sql1 = 'SELECT tag from store_product_tag_master where tag ="'.$tag.'" ';
