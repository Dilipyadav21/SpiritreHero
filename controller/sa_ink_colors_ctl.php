<?php
include_once 'model/sa_ink_colors_mdl.php';

class sa_ink_colors_ctl extends sa_ink_colors_mdl
{
	public $TempSession = "";
	...
	function getAllInkColors(){
		return parent::getAllInkColors_f_mdl();
	}
	
	function deleteInkColor(){
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "delete-ink-col"){
				$id = parent::getVal("icId");
				
				parent::deleteInkColor_f_mdl($id);
			}
		}
	}
	
	function inkColorsPagination(){
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "inkcolors_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				if(isset($keyword) && !empty($keyword)){
					$cond_keyword = "AND (
							ink_color_name LIKE '%".trim($keyword)."%'
						)";
				}
				...
				$sql="
						SELECT count(id) as count FROM store_ink_colors_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id, ink_color,ink_color_name, status FROM store_ink_colors_master WHERE 1
						$cond_keyword
				...
				if(isset($_POST['pagination_export']) && $_POST['pagination_export']=='Y'){
					...
					if(!empty($all_list))
					{
						$sr = $sr_start;
							...
							$html .= '<td><div class="col-preview" style="background-color: '.	$single["ink_color"].'"></div>'.$single["ink_color_name"].'</td>';
							...
							$html .= '<td><button type="button" class="btn btn-primary waves-effect waves-classic btn-addedit-ink-col" data-href="sa-addedit-ink-color.php?stkn='.parent::getVal("stkn").'&icid='.$single["id"].'">Edit</button><button type="button" class="btn btn-danger waves-effect waves-classic btn-confirm-delete-ink-col" data-id="'.$single["id"].'">Delete</button></td>';
