<?php
include_once 'model/sa_roles_mdl.php';

class sa_roles_ctl extends sa_roles_mdl
{
	public $TempSession = "";
	...
	function getAllRoles(){
		return parent::getAllRoles_f_mdl();
	}
	
	function deleteRoles(){
		if(parent::isPOST()){
			if(!empty(parent::getVal("method")) && parent::getVal("method") == "delete-role"){
				$id = parent::getVal("rId");
				
				parent::deleteRoles_f_mdl($id);
			}
		}
	}
	
	function rolesPagination(){
		if(parent::isPOST()){
			if(parent::getVal("hdn_method") == "roles_pagination")
			{
				$record_count=0;
				$page=0;
				$current_page=1;
				...
				$sql="
						SELECT count(id) as count FROM store_owner_role_master WHERE 1
						$cond_keyword
					";
				$all_count = parent::selectTable_f_mdl($sql);
				
				$sql1="
						SELECT id, role_type, role_short_code, status FROM store_owner_role_master WHERE 1
						$cond_keyword
				...
				if(isset($_POST['pagination_export']) && $_POST['pagination_export']=='Y'){
					...
					if(!empty($all_list))
					{
						$sr = $sr_start;
							...
							$html .= '<tr>';
							$html .= '<td>'.$sr.'</td>';
							$html .= '<td>'.$single["role_type"].'</td>';
							
							$html .= '<td>'.$single["role_short_code"].'</td>';
							...
							$html .= '<td><button type="button" class="btn btn-primary waves-effect waves-classic btn-addedit-roles" data-href="sa-addedit-roles.php?stkn='.parent::getVal("stkn").'&rid='.$single["id"].'">Edit</button><button type="button" class="btn btn-danger waves-effect waves-classic btn-confirm-delete-roles" data-id="'.$single["id"].'">Delete</button></td>';
